...
/* Colours are set through `style`, not the `fill` attribute: a custom property
   is only guaranteed to resolve as a CSS value. */
const WHITE = { fill: 'var(--color-neutral-white)' }
...
const KINDS: Record<FileKind, { tint: string; ink: string }> = {
  text: { tint: 'var(--color-brand-light)', ink: 'var(--color-accent-red)' },
  image: { tint: 'var(--color-category-green-light)', ink: 'var(--color-category-green-mid)' },
  font: { tint: 'var(--color-category-purple-light)', ink: 'var(--color-category-purple-mid)' },
  archive: { tint: 'var(--color-neutral-gray-100)', ink: 'var(--color-brand-mid)' },
  design: { tint: 'var(--color-category-blue-light)', ink: 'var(--color-category-blue-mid)' },
  generic: { tint: 'var(--color-neutral-gray-100)', ink: 'var(--color-neutral-black-60)' },
}
...
function Page({ ink }: { ink: string }) {
  return (
    <>
      <path d="M8 5.5h5.2L17 9.3V18a1.2 1.2 0 0 1-1.2 1.2H8A1.2 1.2 0 0 1 6.8 18V6.7A1.2 1.2 0 0 1 8 5.5z" style={{ fill: ink }} />
      <path d="M13.2 5.5 17 9.3h-2.6a1.2 1.2 0 0 1-1.2-1.2V5.5z" style={WHITE} fillOpacity="0.45" />
    </>
  )
}
...
export function FileTypeIcon({ filename, size = 30 }: { filename: string; size?: number }) {
  ...
  return (
    <svg
      ...
      focusable="false"
    >
      <rect width="24" height="24" rx="7" style={{ fill: tint }} />

      {kind === 'text' && (
        <>
          <Page ink={ink} />
          <rect x="8.8" y="12" width="6.4" height="1.5" rx="0.75" style={WHITE} />
          <rect x="8.8" y="15" width="4.2" height="1.5" rx="0.75" style={WHITE} />
        </>
      )}

      {kind === 'image' && (
        <>
          <rect x="5.6" y="7" width="12.8" height="10" rx="2.2" style={{ fill: ink }} />
          <circle cx="9.4" cy="10.5" r="1.35" style={WHITE} />
          <path
      ...
      {kind === 'archive' && (
        <>
          <rect x="5.8" y="6.6" width="12.4" height="10.8" rx="2.2" style={{ fill: ink }} />
          <rect x="11.1" y="6.6" width="1.8" height="4.6" style={WHITE} fillOpacity="0.9" />
          <rect x="10.7" y="11.6" width="2.6" height="3.4" rx="0.9" style={WHITE} />
        </>
      )}

      {kind === 'design' && (
        <>
          <circle cx="9.2" cy="9.4" r="3.3" style={{ fill: ink }} />
          <path d="M15.4 5.6l3.1 5.4h-6.2z" style={{ fill: ink }} fillOpacity="0.75" />
          <rect x="12.4" y="12.8" width="6" height="6" rx="1.4" style={{ fill: ink }} fillOpacity="0.9" />
        </>
      )}
